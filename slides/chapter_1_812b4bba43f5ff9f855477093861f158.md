...
`@script`
You might be asking what thresholding algorithm would be better to use. It's okay if you don't know in deep how these methods work, scikitimage includes a function to evaluate algorithms so you can choose the one that best suits you. 
...
`@script`
Here, we are using a bimodal histogram method: the minimum algorithm. It takes a histogram of the image and smooths it repeatedly until there are only two peaks in it.
...
`@script`
We can calculate a possible optimal threshold with the function threshold_otsu, this is a clustering-based method that does this by maximizing the variance between two classes of pixels, which are separated by the threshold. Equivalently, this threshold minimizes the intra-class variance.
