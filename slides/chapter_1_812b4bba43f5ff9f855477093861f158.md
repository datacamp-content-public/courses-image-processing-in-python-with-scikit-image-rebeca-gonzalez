...
Histogram-based. Where the histogram is a graphical representation of the frequency distribution table of the image and its density.
...
Which it takes a histogram of the image and smooths it repeatedly until there are only two peaks in it.



This is done with the threshold_minimum function from the scikit-image filters module.
...
`@script`
We can calculate a possible optimal threshold with the threshold_otsu function also from the filters module.

This is a clustering-based method that does this by maximizing the variance between two classes of pixels, which are separated by the threshold.

So the thresh here, will be
