...
Works its best in grayscale images because these only use one channel.
...
First we need an image to work on, for this we'll use a module from scikitimage that includes many for testing reasons. We import the module, then choose a grayscaled image: camera, set the thresh value to 127, which is a midpoint between 0 and 255.
...
Methods as Isodata, Li iterative Minimum Cross Entropy and otsu are included in scikit-image.
...
While histogram equalization is a technique used to enhance contrast. Which spreads out the most frequent intensity values in an image.

Here we use equalize_hist and equalize_adapthist from exposure module.
